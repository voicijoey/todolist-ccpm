...
const taskQueryValidation = [
  ...
  query('sort')
    .optional()
    .isIn(['due_date', 'created_at', 'title', 'priority', 'completed'])
    .withMessage('Sort must be one of: due_date, created_at, title, priority, completed'),
  query('order')
    .optional()
    .isIn(['asc', 'desc'])
...
const bulkOperationValidation = [
  ...
  body('updates.priority')
    .optional()
    .isInt({ min: 1, max: 5 })
    .withMessage('Updates.priority must be between 1 and 5')
];
 ...
 * @query   sort - Sort field (due_date, created_at, title, priority, completed)
 ...
 * @body    priority - Task priority (optional, 1-5, default: 1)
 ...
 * @body    priority - Task priority (optional, 1-5)
 * @body    dueDate - Due date (optional, ISO 8601 format)
 */
router.put('/:id',
  validationRules.idParam,
